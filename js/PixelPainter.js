...
function colorPickerRowsAndColumns(x, y){
    ...
    let cache = document.getElementById('colorPicker').getElementsByTagName('td');
    for(var k = 0; k < cache.length; k++){
        cache[k].style.backgroundColor = colorsArray[k];
    }
}
...
for (var t = 0; t < images.length; t++){
    images[t].onclick = function(){
        let pic = this.src;
        document.getElementById('middleColumn').style.backgroundImage = "url(" + pic + ")";
    }
}