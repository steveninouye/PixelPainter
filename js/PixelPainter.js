...
//adds cells into the row
            document.getElementById('row' + i).appendChild(makeElem('td', i + "_" + h, " ", 'tt'));
        }
    }
}
...
document.getElementById('leftColumn').appendChild(makeElem('input','code'));
document.getElementById('leftColumn').appendChild(makeElem('button','save', 'SAVE'));
document.getElementById('leftColumn').appendChild(makeElem('button','colorIn', 'COLOR IN'));
document.getElementById('save').onclick = function(){
    let array = document.getElementById('middleColumn').style.backgroundImage.split('/');
    let picFilename = array[array.length-1];
    session[0] = picFilename.slice(0, picFilename.length-2);
    let hexCode = [];
    document.querySelectorAll(".tt").forEach(element => {
        let hex = rgb2hex(element.style.backgroundColor);
        hexCode.push(hex);
    });
    session[1] = hexCode;
    document.getElementById('code').value = JSON.stringify(session);
}

document.getElementById('colorIn').onclick = function(){
    let loadSession = JSON.parse(document.getElementById('code').value);
    console.log(loadSession[0]);
    document.getElementById('middleColumn').style.backgroundImage = "url('pictures/" + loadSession[0] + "')";
    for(let q = 0; q < document.querySelectorAll(".tt").length; q++){
        document.querySelectorAll(".tt")[q].style.backgroundColor = loadSession[1][q];
    }
}